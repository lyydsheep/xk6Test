...
export default function () {
  ...
  const path = `/api/v1/payments/subscription/${paymentRequestId}`;
  const method = "POST";
  ...
  const jsonReq = JSON.stringify({
    ...
    order: {
      ...
      buyer: {
        referenceBuyerId: "cidtest_notifyservice",
        buyerName: {
          fullName: "fullName",
          firstName: "firstName",
          lastName: "lastName",
  ...
  check(res, {
      ...
      statusUpdating:(r)=> r.json().result.resultStatus==="U"
    });
}
